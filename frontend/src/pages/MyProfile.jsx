...
const MyProfile = () => {
  ...
  // API CALL: function to update user profile
  const updateUserProfileData =async () => {
    ...
    try {
      ...
      if(data.success) {
        ...
        setIsEdit(false)              // so that we come out of the edit mode
        setImage(false)
      } else {
  ...
  // if userData is present, this ui will be available
  return userData && (
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
      {
        // if edit button is clicked, remove the image. After that show the image
        isEdit
        ? <label htmlFor="image">
      ...
      {
        isEdit 
        // updates only name & keep all other data as it is & binds with the userdata state
        ? <input className='bg-gray-50 text-3xl font font-medium max-w-60 mt-4' type="text" value={userData.name} onChange={e => setUserData(prev => ({...prev, name: e.target.value}) )}/> 
      ...
      <div>
        ...
        <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700'>
          <p className='font-medium'>Email id:</p>       {/* Non editable field */}
